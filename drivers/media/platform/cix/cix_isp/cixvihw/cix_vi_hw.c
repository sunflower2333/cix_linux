...
u32 cix_ahb_dphy_read_reg(u32 reg_addr)
{
	...
	if ((reg_addr >= AHB_DPHY0_REG_BASE) && (reg_addr <= AHB_DPHY0_REG_END)) {
		offset = reg_addr - AHB_DPHY0_REG_BASE;
		phy_id = 0;
	} else if ((reg_addr >= AHB_DPHY1_REG_BASE) && (reg_addr <= AHB_DPHY1_REG_END)) {
...
u32 cix_ahb_csi_read_reg(u32 reg_addr)
{
	...
	if ((reg_addr >= AHB_CSI0_REG_BASE) && (reg_addr <= AHB_CSI0_REG_END)) {
		...
		csi_id = 0;
	} else if ((reg_addr >= AHB_CSI1_REG_BASE) && (reg_addr <= AHB_CSI1_REG_END)) {
		offset = reg_addr - AHB_CSI1_REG_BASE;
		csi_id = 1;
	} else if ((reg_addr >= AHB_CSI2_REG_BASE) && (reg_addr <= AHB_CSI2_REG_END)) {
		offset = reg_addr - AHB_CSI2_REG_BASE;
		csi_id = 2;
	} else if ((reg_addr >= AHB_CSI3_REG_BASE) && (reg_addr <= AHB_CSI3_REG_END)) {
...
u32 cix_ahb_csidma_read_reg(u32 reg_addr)
{
	...
	if ((reg_addr >= AHB_CSIDMA0_REG_BASE) && (reg_addr <= AHB_CSIDMA0_REG_END)) {
		offset = reg_addr - AHB_CSIDMA0_REG_BASE;
		csidma_id = 0;
	} else if ((reg_addr >= AHB_CSIDMA1_REG_BASE) && (reg_addr <= AHB_CSIDMA1_REG_END)) {
		offset = reg_addr - AHB_CSIDMA1_REG_BASE;
		csidma_id = 1;
	} else if ((reg_addr >= AHB_CSIDMA2_REG_BASE) && (reg_addr <= AHB_CSIDMA2_REG_END)) {
		offset = reg_addr - AHB_CSIDMA2_REG_BASE;
		csidma_id = 2;
	} else if ((reg_addr >= AHB_CSIDMA3_REG_BASE) && (reg_addr <= AHB_CSIDMA3_REG_END)) {
...
u32 cix_ahb_csircsu_read_reg(u32 reg_addr)
{
	...
	if ((reg_addr >= AHB_CSIRCSU0_REG_BASE) && (reg_addr <= AHB_CSIRCSU0_REG_END)) {
		offset = reg_addr - AHB_CSIRCSU0_REG_BASE;
		csircsu_id = 0;
	} else if ((reg_addr >= AHB_CSIRCSU1_REG_BASE) && (reg_addr <= AHB_CSIRCSU1_REG_END)) {
...
void cix_dphy_resets(int id, int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			LOG(LOG_ERR, "First: Failed to deassert cmnrst_phy %d\n", id);
		}
...
void cix_csi_resets(int id, int enable)
{
	...
	if (enable) {
		...
		if (ret) {
			LOG(LOG_ERR, "Second: Failed to assert csi_reset %d before deassert\n", id);
		}
		ret = reset_control_deassert(cix_vi_hw_info->csi_reset[id]);
		if (ret) {
...
void cix_csidma_resets(int id, int enable)
{
	int ret;
	if (enable) {
		ret = reset_control_deassert(cix_vi_hw_info->csibridge_reset[id]);
		if (ret) {
			LOG(LOG_ERR, "First: Failed to deassert csibridge_reset %d\n", id);
		}
		ret = reset_control_assert(cix_vi_hw_info->csibridge_reset[id]);
		if (ret) {
			LOG(LOG_ERR, "Second: Failed to assert csibridge_reset %d\n", id);
		}
		ret = reset_control_deassert(cix_vi_hw_info->csibridge_reset[id]);
		if (ret) {
			LOG(LOG_ERR, "Last: Failed to deassert csibridge_reset %d\n", id);
		}
	} else {
		ret = reset_control_assert(cix_vi_hw_info->csibridge_reset[id]);
		if (ret) {
...
void cix_enable_dphy_clk(u32 reg_addr, u32 value)
{
	...
	if (value > 0) {
		...
		if (ret < 0) {
			LOG(LOG_ERR, "%s, enable phy%d_psm_clk error\n", __func__, dhpy_id);
			goto err_phy_clks;
		}

		ret = clk_prepare_enable(cix_vi_hw_info->phy_apb_clk[dhpy_id]);
		if (ret < 0) {
			LOG(LOG_ERR, "%s, enable phy%d_apb_clk error\n", __func__, dhpy_id);
			goto err_phy_clks;
		}
		...
		{
			clk_disable_unprepare(cix_vi_hw_info->phy_psm_clk[dhpy_id]);
			clk_disable_unprepare(cix_vi_hw_info->phy_apb_clk[dhpy_id]);
	...
	return;
err_phy_clks:
	LOG(LOG_ERR, "%s, enable dphy%d  clk failed\n", __func__, dhpy_id);
}
...
void cix_set_csi_clk_rate(u32 reg_addr, u32 value)
{
	...
	if (value > 0) {
		LOG(LOG_INFO, "%s, enable csi%d_sys_clk %d \n", __func__, csi_id, value);
		ret = clk_prepare_enable(cix_vi_hw_info->csi_sys_clk[csi_id]);
		if (ret < 0) {
			LOG(LOG_ERR, "%s, enable csi%d_sys_clk error\n", __func__, csi_id);
			goto err_csi_clks;
		}

		ret = clk_prepare_enable(cix_vi_hw_info->csi_p_clk[csi_id]);
		if (ret < 0) {
			LOG(LOG_ERR, "%s, enable csi%d_p_clk error\n", __func__, csi_id);
			goto err_csi_clks;
		}
		switch (csi_id) {
		case 0:
			for (i = 0; i < 4; i++) {
				if (clk_prepare_enable(cix_vi_hw_info->csi0_pixel_clk[i])) {
					LOG(LOG_ERR, "csi0_pixel_clk[%d] enable failed\n", i);
				goto err_csi_clks;
				}
			}
			break;
		case 1:
			if (clk_prepare_enable(cix_vi_hw_info->csi1_pixel_clk)) {
		...
		case 2:
			for (i = 0; i < 4; i++) {
				if (clk_prepare_enable(cix_vi_hw_info->csi2_pixel_clk[i])) {
					LOG(LOG_ERR, "csi2_pixel_clk[%d] enable failed\n", i);
					goto err_csi_clks;
				}
			}
			break;
		case 3:
			if (clk_prepare_enable(cix_vi_hw_info->csi3_pixel_clk)) {
				LOG(LOG_ERR, "csi3_pixel_clk[%d] enable failed\n");
				goto err_csi_clks;
			}
			break;
		}
		...
		if (csi_power_status[csi_id] == POWER_ON)
		{
			...
			case 0:
				for (i = 0; i < 4; i++)
					clk_disable_unprepare(cix_vi_hw_info->csi0_pixel_clk[i]);
				break;
			case 1:
			...
			case 2:
				for (i = 0; i < 4; i++)
					clk_disable_unprepare(cix_vi_hw_info->csi2_pixel_clk[i]);
				break;
			case 3:
	...
	return;
err_csi_clks:
	LOG(LOG_ERR, "%s, enable csi%d  clk failed\n", __func__, csi_id);
}
void cix_enable_csidma_clk(u32 reg_addr, u32 value)
{
	...
	if (value > 0) {
		ret = clk_prepare_enable(cix_vi_hw_info->csidma_apbclk[csidma_id]);
		if (ret < 0) {
			LOG(LOG_ERR, "%s, enable csidma%d_apbclk error\n", __func__, csidma_id);
		}
		...
		{
			clk_disable_unprepare(cix_vi_hw_info->csidma_apbclk[csidma_id]);
...
static irqreturn_t mipi_csi2_irq_handler(int irq, void *base)
{
	...
	printk(KERN_INFO "csi irq enter status %x \n",status);
...
static irqreturn_t mipi_csi2_err_irq_handler(int irq, void *base)
{
	...
	printk(KERN_INFO "CSI ERROR status 0x%08x, DPHY error status 0x%08x\n",
...
static int cix_vi_hw_probe(struct platform_device *pdev)
{
	...
	cix_vi_hw_info = devm_kzalloc(dev, sizeof(struct cix_vi_hw_dev), GFP_KERNEL);
	if (!cix_vi_hw_info)
		return -ENOMEM;
	...
	for (i = 0; i < DPHY_NUM; i++) {
		...
		cix_vi_hw_info->phy_psm_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
	...
	for (i = 0; i < DPHY_NUM; i++) {
		...
		cix_vi_hw_info->phy_apb_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
	...
	for (i = 0; i < CSI_NUM; i++) {
		...
		cix_vi_hw_info->csi_p_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
	...
	for (i = 0; i < CSI_NUM; i++) {
		...
		cix_vi_hw_info->csi_sys_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
	...
	for (i = 0; i < CSI0_PIXEL_NUM; i++) {
		...
		cix_vi_hw_info->csi0_pixel_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
		if (IS_ERR(cix_vi_hw_info->csi0_pixel_clk[i])) {
			LOG(LOG_ERR, "Couldn't get %s\n", clk_name);
			return PTR_ERR(cix_vi_hw_info->csi0_pixel_clk[i]);
		}
	}
	cix_vi_hw_info->csi1_pixel_clk = devm_clk_get_optional(&pdev->dev, "csi1_p0clk");
	...
	for (i = 0; i < CSI2_PIXEL_NUM; i++) {
		...
		cix_vi_hw_info->csi2_pixel_clk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
		if (IS_ERR(cix_vi_hw_info->csi2_pixel_clk[i])) {
			LOG(LOG_ERR, "Couldn't get %s\n", clk_name);
			return PTR_ERR(cix_vi_hw_info->csi2_pixel_clk[i]);
		}
	}
	cix_vi_hw_info->csi3_pixel_clk = devm_clk_get_optional(&pdev->dev, "csi3_p0clk");
	...
	for (i = 0; i < CSIDMA_NUM; i++) {
		...
		cix_vi_hw_info->csidma_apbclk[i] = devm_clk_get_optional(&pdev->dev, clk_name);
	...
	for (i = 0; i < DPHY_NUM; i++) {
			...
			cix_vi_hw_info->rst_dphy[i] = devm_reset_control_get_optional_shared(&pdev->dev, reset_name);
			if (IS_ERR(cix_vi_hw_info->rst_dphy[i])) {
				if (PTR_ERR(cix_vi_hw_info->rst_dphy[i]) != -EPROBE_DEFER)
					LOG(LOG_ERR, "Failed to get %s\n", reset_name);
				return PTR_ERR(cix_vi_hw_info->rst_dphy[i]);
			}
			sprintf(reset_name, "phy%d_cmnrst", i);
			cix_vi_hw_info->cmnrst_phy[i] = devm_reset_control_get_optional_shared(&pdev->dev, reset_name);
			if (IS_ERR(cix_vi_hw_info->cmnrst_phy[i])) {
				if (PTR_ERR(cix_vi_hw_info->cmnrst_phy[i]) != -EPROBE_DEFER)
					LOG(LOG_ERR, "Failed to get %s\n", reset_name);
	...
	for (i = 0; i < CSI_NUM; i++) {
		...
		cix_vi_hw_info->csi_reset[i] = devm_reset_control_get_optional_shared(&pdev->dev, reset_name);
		if (IS_ERR(cix_vi_hw_info->csi_reset[i])) {
			if (PTR_ERR(cix_vi_hw_info->csi_reset[i]) != -EPROBE_DEFER)
				LOG(LOG_ERR, "Failed to get %s\n", reset_name);
	...
	for (i = 0; i < CSIDMA_NUM; i++) {
		...
		cix_vi_hw_info->csibridge_reset[i] = devm_reset_control_get_optional_shared(&pdev->dev, reset_name);
		if (IS_ERR(cix_vi_hw_info->csibridge_reset[i])) {
			if (PTR_ERR(cix_vi_hw_info->csibridge_reset[i]) != -EPROBE_DEFER)
				LOG(LOG_ERR, "Failed to get %s\n", reset_name);
	...
	for (i = 0; i < 2; i++) {
		...
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_dphy_base[i]);
		if (ret < 0) {
			cix_vi_hw_info->ahb_dphy_base[i] = 0;
			LOG(LOG_ERR, "failed to get ahb-dphy%d-base.", i);
		}
		sprintf(buffer, "ahb-dphy%d-size", i);
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_dphy_size[i]);
		if (ret < 0) {
	...
	for (i = 0; i < 4; i++) {
		...
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_csi_base[i]);
		if (ret < 0) {
			cix_vi_hw_info->ahb_csi_base[i] = 0;
			LOG(LOG_ERR, "failed to get ahb-csi%d-base.", i);
		}
		sprintf(buffer, "ahb-csi%d-size", i);
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_csi_size[i]);
		if (ret < 0) {
	...
	for (i = 0; i < 4; i++) {
		...
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_csidma_base[i]);
		if (ret < 0) {
			cix_vi_hw_info->ahb_csidma_base[i] = 0;
			LOG(LOG_ERR, "failed to get %s.", buffer);
		}
		sprintf(buffer, "ahb-csidma%d-size", i);
		ret = fwnode_property_read_u32(pdev->dev.fwnode, "ahb-csidma0-size", &cix_vi_hw_info->ahb_csidma_size[i]);
		if (ret < 0) {
	...
	for (i = 0; i < 2; i++) {
		...
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_csircsu_base[i]);
		if (ret < 0) {
			cix_vi_hw_info->ahb_csircsu_base[i] = 0;
			LOG(LOG_ERR, "failed to get ahb-csircsu%d-base.", i);
		}
		sprintf(buffer, "ahb-csircsu%d-size", i);
		ret = fwnode_property_read_u32(pdev->dev.fwnode, buffer, &cix_vi_hw_info->ahb_csircsu_size[i]);
		if (ret < 0) {
	...
	for (i = 0; i < 2; i++) {
		if (cix_vi_hw_info->ahb_dphy_base[i] != 0 && cix_vi_hw_info->ahb_dphy_size[i] != 0) {
			cix_vi_hw_info->ahb_dphy_base_addrs[i] = devm_ioremap(&pdev->dev,
					cix_vi_hw_info->ahb_dphy_base[i], cix_vi_hw_info->ahb_dphy_size[i]);
			if (!cix_vi_hw_info->ahb_dphy_base_addrs[i]) {
				cix_vi_hw_info->ahb_dphy_base_addrs[i] = 0;
				LOG(LOG_ERR, "failed to ioremap ahb-dphy%d register region.", i);
			}
		}
		LOG(LOG_INFO, "ahb-dphy%d-base=0x%x, ahb-dphy%d-size=0x%x ahb_dphy%d_base_addr=%px",
			i, cix_vi_hw_info->ahb_dphy_base[i],
			i, cix_vi_hw_info->ahb_dphy_size[i],
			i, cix_vi_hw_info->ahb_dphy_base_addrs[i]);
	}

	for (i = 0; i < 4; i++) {
		if (cix_vi_hw_info->ahb_csi_base[i] != 0 && cix_vi_hw_info->ahb_csi_size[i] != 0) {
			cix_vi_hw_info->ahb_csi_base_addrs[i] = devm_ioremap(&pdev->dev,
					cix_vi_hw_info->ahb_csi_base[i], cix_vi_hw_info->ahb_csi_size[i]);
			if (!cix_vi_hw_info->ahb_csi_base_addrs[i]) {
				cix_vi_hw_info->ahb_csi_base_addrs[i] = 0;
				LOG(LOG_ERR, "failed to ioremap ahb-csi%d register region.", i);
			}
		}
		...
		ret = devm_request_irq(dev, irq0, mipi_csi2_irq_handler,
				IRQF_ONESHOT | IRQF_SHARED, dev_name(dev), cix_vi_hw_info->ahb_csi_base_addrs[i]);
		if (ret) {
		...
		LOG(LOG_INFO, "ahb-csi%d-base=0x%x, ahb-csi%d-size=0x%x ahb_csi%d_base_addr=%px",
			i, cix_vi_hw_info->ahb_csi_base[i],
			i, cix_vi_hw_info->ahb_csi_size[i],
			i, cix_vi_hw_info->ahb_csi_base_addrs[i]);
	}

	for (i = 0; i < 4; i++) {
		if (cix_vi_hw_info->ahb_csidma_base[i] != 0 && cix_vi_hw_info->ahb_csidma_size[i] != 0) {
			cix_vi_hw_info->ahb_csidma_base_addrs[i] = devm_ioremap(&pdev->dev,
					cix_vi_hw_info->ahb_csidma_base[i], cix_vi_hw_info->ahb_csidma_size[i]);
			if (!cix_vi_hw_info->ahb_csidma_base_addrs[i]) {
				cix_vi_hw_info->ahb_csidma_base_addrs[i] = 0;
				LOG(LOG_ERR, "failed to ioremap ahb-csidma%d register region.", i);
			}
		}
		LOG(LOG_INFO, "ahb-csidma%d-base=0x%x, ahb-csidma%d-size=0x%x ahb_csidma%d_base_addr=%px",
			i, cix_vi_hw_info->ahb_csidma_base[i],
			i, cix_vi_hw_info->ahb_csidma_size[i],
			i, cix_vi_hw_info->ahb_csidma_base_addrs[i]);

	}

	for (i = 0; i < 2; i++) {
		if (cix_vi_hw_info->ahb_csircsu_base[i] != 0 && cix_vi_hw_info->ahb_csircsu_size[i] != 0) {
			cix_vi_hw_info->ahb_csircsu_base_addrs[i] = devm_ioremap(&pdev->dev,
					cix_vi_hw_info->ahb_csircsu_base[i], cix_vi_hw_info->ahb_csircsu_size[i]);
			if (!cix_vi_hw_info->ahb_csircsu_base_addrs[i]) {
				cix_vi_hw_info->ahb_csircsu_base_addrs[i] = 0;
				LOG(LOG_ERR, "failed to ioremap ahb-csircsu% register region.", i);
			}
		}
		LOG(LOG_INFO, "ahb-csircsu%d-base=0x%x, ahb-csircsu%d-size=0x%x ahb_csircsu%d_base_addr=%px",
			i, cix_vi_hw_info->ahb_csircsu_base[i],
			i, cix_vi_hw_info->ahb_csircsu_size[i],
			i, cix_vi_hw_info->ahb_csircsu_base_addrs[i]);
	}
