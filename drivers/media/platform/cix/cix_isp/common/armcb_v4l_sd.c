...
static int armcb_subdev_register_nodes_async(struct v4l2_subdev *sd)
{
	...
	if (res < 0) {
		LOG(LOG_ERR, "v4l2_device_register_subdev failed for %s", sd->name);
		WARN_ON(1);
	}
...
int armcb_camera_async_complete(struct v4l2_async_notifier *notifier)
{
	...
	list_for_each_entry (sd, &v4l2_dev->subdevs, list) {
		...
		LOG(LOG_INFO, "step6 name(%s) grp_id(%d)", sd->name, sd->grp_id);
		nr = (sd->grp_id < (u32)ARMCB_SUBDEV_NODE_IDX_END) ? sd->grp_id : nr;
		ret = __video_register_device(vdev, VFL_TYPE_SUBDEV, nr, 1, sd->owner);
		if (ret < 0) {
			kfree(vdev);
			goto clean_up;
		}
	...
	list_for_each_entry (sd, &v4l2_dev->subdevs, list) {
		vdev = sd->devnode;
		if (!vdev) {
			LOG(LOG_ERR, "subdev node %s register failed.", sd->name);
			continue;
		}
		...
		ret = media_device_register_entity(v4l2_dev->mdev,
							&vdev->entity);
		if (ret < 0) {
			LOG(LOG_ERR, "register media entity for subdev %s failed.",
			       vdev->dev.kobj.name);
			goto clean_up;
		}
		LOG(LOG_INFO,
...
int armcb_subdev_register(struct armcb_sd_subdev *armcb_sdreg, unsigned int cam_id)
{
