...
const char *const log_level_name[SYSTEM_LOG_LEVEL_MAX] = {"DEBUG", "INFO", "NOTIC", "WARN", "ERR", "CRIT"};
//debug log names for modules
const char *const log_module_name[SYSTEM_LOG_MODULE_MAX] = {"GENERIC","COMMON","SENSOR","ISP",};
...
void _acbcamera_log_write_ext( const char *const func, const char *const file, const unsigned line,
	...
	size= SYSTEM_SNPRINTF( _acbcamera_logger_buf, ACBCAMERA_BUFFER_SIZE, "%s %5u %5u <%-8s> [%-5s] %s %s (%u): ",
		timestamp, current->pid, current->tgid, log_module_name[log_module], log_level_name[log_level], filename_short( file ), func, line);
	SYSTEM_VSNPRINTF(_acbcamera_logger_buf + size , ACBCAMERA_BUFFER_SIZE-size, fmt, vaa);
...
void _acbcamera_log_write( const char *const func, const char *const file, const unsigned line,
							const uint32_t log_level, const uint32_t log_module,
							const uint32_t ratelimited, const char *const fmt, ... ) {
	va_list va;
	va_start( va, fmt );
	_acbcamera_log_write_ext( func, file, line, log_level, log_module, ratelimited, fmt, va );
	va_end( va );
}
