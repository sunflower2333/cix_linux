...
static struct rdr_exception_info_s g_isp_einfo[] = {
	{ { 0, 0 }, RDR_ISP_SOC_ISR_ERR_MODID, RDR_ISP_SOC_ISR_ERR_MODID, RDR_ERR,
	 RDR_REBOOT_NO, RDR_ISP, RDR_ISP, RDR_ISP,
	 (u32)RDR_REENTRANT_DISALLOW, ISP_S_EXCEPTION, 0, (u32)RDR_UPLOAD_YES,
	 "isp", "isp isr proc", 0, 0, 0 },
};
 ...
 */
static void cix_isp_rproc_rdr_dump(u32 modid, u32 etype,
...
static void cix_isp_rproc_rdr_register_exception(void)
{
	...
	for (i = 0; i < sizeof(g_isp_einfo) / sizeof(struct rdr_exception_info_s); i++) {
		LOG(LOG_DEBUG, "register exception:%u", g_isp_einfo[i].e_exce_type);
		ret = rdr_register_exception(&g_isp_einfo[i]);
		if (ret == 0) {
			LOG(LOG_ERR, "rdr_register_exception fail, ret = [%d]\n", ret);
			return;
		}
	}
}
...
static void cix_isp_rproc_rdr_unregister_exception(void)
{
	...
	for (i = 0; i < sizeof(g_isp_einfo) / sizeof(struct rdr_exception_info_s); i++) {
		LOG(LOG_DEBUG, "unregister exception:%u", g_isp_einfo[i].e_exce_type);
...
static int cix_isp_rproc_rdr_register_core(void)
{
	...
	LOG(LOG_DEBUG, "%s,%d: addr=0x%llx   [0x%llx], len=0x%x\n",
		__func__, __LINE__, g_current_info.log_addr, g_isp_addr, g_current_info.log_len);
...
static armcb_v4l2_config_dev_t *p_v4l_config_dev = NULL;
...
static int armcb_get_vout_addr_by_outport_i5(uint32_t outport, uint32_t *p_vout_reg1, uint32_t *p_vout_reg2)
{
	int rc = 0;
...
static int armcb_get_vin_addr_by_outport_i7(uint32_t outport, uint32_t *p_vin_daw_reg, uint32_t *p_vin_dar_reg)
{
	int rc = 0;
...
static int armcb_get_output_addr_by_outport_i7(uint32_t outport, uint32_t *p_vout_reg1, uint32_t *p_vout_reg2)
{
	int rc = 0;
	uint32_t vout_reg1 = 0;
	uint32_t vout_reg2 = 0;
...
int armcb_v4l2_config_update_stream_vin_addr(armcb_v4l2_stream_t *pstream)
{
	...
	if (armcb_get_vin_addr_by_outport_i7(pstream->outport, &vin_daw_reg, &vin_dar_reg) < 0) {
		LOG( LOG_DEBUG, "ctx_id:%d stream_id:%d failed to get reg for outport:%d",
	...
	if ( !list_empty( &pstream->stream_buffer_list ) ) {
		pbuf = list_entry( pstream->stream_buffer_list.next, armcb_v4l2_buffer_t, list );
		list_del( &pbuf->list );
		list_add_tail( &pbuf->list, &pstream->stream_buffer_list_busy );
	}

	if ( !pbuf ) {
		LOG( LOG_DEBUG, "[Stream#%d] no empty buffers", pstream->stream_id );
		startaddr = pstream->reserved_buf_addr;
	} else {
		startaddr = (unsigned int )(uintptr_t)vb2_plane_vaddr(&pbuf->vvb.vb2_buf, 0);
	}

	if (vin_dar_reg && vin_daw_reg && startaddr) {
		LOG( LOG_DEBUG, "[Stream#%d] set daw0(0x%x) & dar0(0x%x) startaddr(0x%x) reserved_buf_addr(0x%x) VIN_BUFFER_LONG_ADDR(0x%x)",
			pstream->stream_id, vin_daw_reg, vin_dar_reg, startaddr, pstream->reserved_buf_addr, VIN_BUFFER_LONG_ADDR);
...
int armcb_v4l2_config_update_stream_hw_addr(armcb_v4l2_stream_t *pstream)
{
	...
	if (armcb_get_output_addr_by_outport_i7(pstream->outport, &vout_reg1, &vout_reg2) < 0) {
		LOG( LOG_DEBUG, "ctx_id:%d stream_id:%d failed to get reg for outport:%d",
			pstream->ctx_id, pstream->stream_id, pstream->outport);
		return -EINVAL;
	}
	...
	if (vout_reg1 && startaddr) {
		LOG( LOG_DEBUG, "[Stream#%d] set addr: 0x%x = 0x%x", pstream->stream_id,
			vout_reg1, startaddr);
...
static void armcb_isp_irq_sol_i5(unsigned int ctx_id)
{
	...
	for (i = 0; i < V4L2_STREAM_TYPE_MAX; i++) {
		...
		if (armcb_get_vout_addr_by_outport_i5(pstream->outport, &vout_reg1, &vout_reg2) < 0) {
			LOG( LOG_ERR, "ctx_id:%d stream_id:%d failed to get vout reg for outport:%d",
		...
		if ((v4l2_fmt->fmt.pix_mp.pixelformat == V4L2_PIX_FMT_NV21 ||
			v4l2_fmt->fmt.pix_mp.pixelformat == V4L2_PIX_FMT_NV12) && startaddr) {
			armcb_isp_write_reg(vout_reg1, startaddr);
		}
	}
}
...
static void armcb_isp_irq_sol_i7(unsigned int ctx_id)
{
	...
	for (i = 0; i < V4L2_STREAM_TYPE_MAX; i++) {
		...
		LOG( LOG_DEBUG, "sol cxt_id:%d, stream_id:%d", pstream->ctx_id, pstream->stream_id);
		/* check if stream is on */
		if ( !pstream->stream_started ) {
			LOG( LOG_DEBUG, "[Stream#%d] stream video is not started yet on ctx %d",
				pstream->stream_id, ctx_id );
			continue;
		}

		if (armcb_v4l2_config_update_stream_hw_addr(pstream)) {
			LOG( LOG_DEBUG, "[Stream#%d] failed to update stream hw addr for cxt_id:%d", pstream->stream_id, ctx_id);
		}
	}
}
...
static void armcb_isp_irq_sof_i7(unsigned int ctx_id)
{
	...
	for (i = 0; i < V4L2_STREAM_TYPE_MAX; i++) {
		...
		if (armcb_v4l2_config_update_stream_vin_addr(pstream)) {
			LOG( LOG_DEBUG, "[Stream#%d] failed to update stream vin addr for cxt_id:%d", pstream->stream_id, ctx_id);
		}
	}
}

static void armcb_v4l2_config_queue_event_with_status(
	...
	unsigned int status)
{
	...
	if (pData[5] % 30 == 0) {
		LOG(LOG_DEBUG, "isp_queue event id(%d) type(%d) mask=0x%x, "\
				"status=0x%x, id = 0x%x, sen_id = 0x%x, frm_cnt_sof = %u, frm_cnt_sol = %u",
				ev.id, ev.type, pData[0], pData[1], pData[2], pData[3], pData[4], pData[5]);
...
static void armcb_v4l2_config_queue_event(
	struct video_device  *pvdev,
	struct isp_irq_info  *pirq_info)
{
	unsigned int *pData = NULL;
...
static inline void armcb_i5_afbc_err_process(struct isp_irq_info *pirq_info)
{
	...
	if (pirq_info->status & I5_INT_AFBC_MASK) {
		...
		if (pirq_info->err_detail_status[0] & I5_INT_AFBC_DETAIL_ERR_MASK) {
			afbc_clean = armcb_isp_read_reg(I5_INT_AFBC_CLEAR_ADDR);
			armcb_isp_write_reg(I5_INT_AFBC_CLEAR_ADDR,
				afbc_clean | (I5_INT_AFBC_MASK & pirq_info->err_detail_status[0]));
		}
	}
}
...
static irqreturn_t armcb_I5_isp_isr(s32 irq, void *pdev)
{
	...
	if(irq_info.status) {
		...
		LOG(LOG_DEBUG, "irq: frm_cnt=%u,  status=0x%x", irq_info.frm_cnt_sol,
			irq_info.status);
		...
		if (irq_info.status & I5_INT_VOUTX_INT_MASK) {
			...
			if (irq_info.status & I5_INT_VOUT0_DONE) {
				...
				armcb_v4l2_config_queue_event(pvdev, &irq_info_vout);
 ...
 */
static inline void
armcb_i7_get_sof_frm_cnt(struct isp_irq_info *pirq_info,
	...
	/*VIN_OUT_SOF: SOF is generated by vin output end, isp can control*/
		...
		* bits[1:0]:vin_frame_index*/
		pirq_info->sen_id =
			(pint_id->cxt_idx << cxt_offset) | (pint_id->frm_idx);
...
static inline void armcb_i7_afbc_err_process(struct isp_irq_info *pirq_info)
{
	...
	if (pirq_info->status & I7_INT_AFBC_ERR_MASK) {
		...
		if (afbc_status & I7_INT_AFBC_DETAIL_ERR_MASK) {
			...
			armcb_isp_write_reg(I7_INT_AFBC_CLEAR_ADDR,
				afbc_clr | (I7_INT_AFBC_DETAIL_ERR_MASK & afbc_status));
		}
	}
}
...
static irqreturn_t armcb_I7_isp_isr(s32 irq, void *pdev)
{
	...
	if (irq_info.status) {
		...
		LOG(LOG_DEBUG, "status=0x%x, id=0x%x, sel=0x%x, sof=%d, sol=%d-%d, 3a=%d",
			irq_info.status,
			irq_info.id,
			frame_cnt_sel.val,
			irq_info.frm_cnt_sof,
			irq_info.frm_cnt_sol,
			irq_info.frm_cnt_nxt_sol,
			irq_info.frm_cnt_3a);

		/*2.4 set vout buffer address*/
		if (irq_info.status & I7_INT_SOL_MASK) {
			next_ctx_id =
				((int_id.cxt_nxt_idx << cxt_offset) |
		...
		if (irq_info.status & I7_INT_VIN_CHNL_L_BUF_DONE) {
			...
			reg_val &= 0xfffeffff;/*r_dump_en = 0 disable daw0 write*/
		...
		if (irq_info.status & I7_IRQ_EVENT_POST_MASK) {
			bh_info = armcb_irq_msg_ev_alloc(&p_v4l_config_dev->isr_fh);
...
static irqreturn_t armcb_I7_isp_err_isr(s32 irq, void *pdev)
{
	...
	unsigned int             irq_clear          = 0;
	#ifdef CONFIG_PLAT_BBOX
	const char start_flag_str[]                 = "ispDumpStart";
	int start_flag_len                          = sizeof(start_flag_str) - 1;
	int start_flag_num                          = (start_flag_len % 4 ? 1 : 0) + start_flag_len / 4;
	int total_reg_num                           = ERR_DETAIL_NUM + ISP_TOP_NUM + \
		ISP_VIN_NUM + ISP_IFBC_NUM * 2 + ISP_PSC_NUM + ISP_SCA_NUM + start_flag_num;
	u64 err_save_base_addr[ISP_SAVE_MAX]        = {I7_TOP_BASE_ADDR, I7_VIN_BASE_ADDR, \
		I7_IFBC_BASE_ADDR, I7_IFBD_BASE_ADDR, I7_PSC_BASE_ADDR, I7_SCA_BASE_ADDR};
	u64 write_shift                             = g_isp_addr + rdr_writen_num * 4;
	err_save_module err_save_len[ISP_SAVE_MAX]  = {ISP_TOP_NUM, ISP_VIN_NUM, \
					ISP_IFBC_NUM, ISP_IFBC_NUM, ISP_PSC_NUM, ISP_SCA_NUM};
	u32 val;
	int i,j;
	...
	if (irq_info.status) {
		...
		irq_info.err_detail_status[0]    = armcb_isp_read_reg(I7_INT_DETAIL_ERR_ADDR1);
		irq_info.err_detail_status[1]    = armcb_isp_read_reg(I7_INT_DETAIL_ERR_ADDR2);
		irq_info.err_detail_status[2]    = armcb_isp_read_reg(I7_INT_DETAIL_ERR_ADDR3);
		irq_info.err_detail_status[3]    = armcb_isp_read_reg(I7_INT_DETAIL_ERR_ADDR4);
		LOG(LOG_ERR, "status=0x%x, dtail= {0x%x, 0x%x, 0x%x, 0x%x}",
			irq_info.status, irq_info.err_detail_status[0],
			irq_info.err_detail_status[1], irq_info.err_detail_status[2],
		...
		if (irq_info.status) {
			 ...
			 */
			if ((rdr_writen_num + total_reg_num) * 4 <  g_current_info.log_len) {
				memcpy((void*)write_shift, start_flag_str, start_flag_len);
				write_shift    += start_flag_num * 4;
				rdr_writen_num += start_flag_num;

				writel(irq_info.status, (volatile void __iomem *)write_shift);
				writel(irq_info.err_detail_status[0], (volatile void __iomem *)(write_shift + 0x4));
				writel(irq_info.err_detail_status[1], (volatile void __iomem *)(write_shift + 0x8));
				writel(irq_info.err_detail_status[2], (volatile void __iomem *)(write_shift + 0xc));
				writel(irq_info.err_detail_status[3], (volatile void __iomem *)(write_shift + 0x10));
				write_shift += ERR_DETAIL_NUM * 4;

				for (i = 0; i < ISP_SAVE_MAX; i++) {
					for(j = 0; j < err_save_len[i]; j++) {
						val = armcb_isp_read_reg(err_save_base_addr[i] + 4 * j);
						writel(val, (volatile void __iomem *)(write_shift + 4 * j));
					}
...
static int armcb_isp_interrupt_task(void *data) {
	...
	while (1) {
		...
		if (irq_info.status) {
			...
			LOG(LOG_DEBUG, "status=0x%x, id=0x%x, sof=%d, sol=%d-%d, 3a=%d",
				irq_info.status,
				irq_info.id,
				irq_info.frm_cnt_sof,
				irq_info.frm_cnt_sol,
			...
			if (irq_info.status & I7_INT_VOUTX_INT_MASK) {
				if (irq_info.status & I7_INT_VOUT1_BUF_DONE) {
					outport = (1 << ISP_OUTPUT_PORT_VOUT1);
					if (armcb_v4l2_find_ctx_stream_by_outport(outport, &ctx_id, &stream_id) < 0) {
						LOG(LOG_ERR, "failed to find a valid ctx_id and stream_id for port %d", outport);
						goto loop;
					}
					armcb_isp_put_frame(ctx_id, stream_id, ISP_OUTPUT_PORT_VOUT1);
					irq_info_vout = irq_info;
					irq_info_vout.status = I7_INT_VOUT1_BUF_DONE;
					armcb_v4l2_config_queue_event(pvdev, &irq_info_vout);

					irq_info.status &= ~I7_INT_VOUT1_BUF_DONE;
				}
				if (irq_info.status & I7_INT_VOUT3_BUF_DONE) {
					...
					armcb_isp_put_frame(ctx_id, stream_id, ISP_OUTPUT_PORT_VOUT3);
					irq_info_vout = irq_info;
					irq_info_vout.status = I7_INT_VOUT3_BUF_DONE;
					armcb_v4l2_config_queue_event(pvdev, &irq_info_vout);

					irq_info.status &= ~I7_INT_VOUT3_BUF_DONE;
				}
				if (irq_info.status & I7_INT_VOUT5_BUF_DONE) {
					...
					armcb_isp_put_frame(ctx_id, stream_id, ISP_OUTPUT_PORT_VOUT5);
					irq_info_vout = irq_info;
					irq_info_vout.status = I7_INT_VOUT5_BUF_DONE;
					armcb_v4l2_config_queue_event(pvdev, &irq_info_vout);

					irq_info.status &= ~I7_INT_VOUT5_BUF_DONE;
				}
				if (irq_info.status & I7_INT_VOUT7_BUF_DONE) {
					...
					armcb_isp_put_frame(ctx_id, stream_id, ISP_OUTPUT_PORT_VOUT7);
					irq_info_vout = irq_info;
					irq_info_vout.status = I7_INT_VOUT7_BUF_DONE;
					armcb_v4l2_config_queue_event(pvdev, &irq_info_vout);

					irq_info.status &= ~I7_INT_VOUT7_BUF_DONE;
				}
			}
...
static int armcb_v4l2_config_fop_release(struct file *file)
{
	...
	if (fh) {
		...
		LOG(LOG_DEBUG, "armcb_fop_release v4l2_event_unsubscribe, ret = %d", ret);
...
static ssize_t armcb_v4l2_config_fop_read(struct file *filep,
...
static int armcb_v4l2_config_fop_mmap(struct file *file, struct vm_area_struct *vma)
{
	return 0;
}
...
static int armcb_v4l2_config_subscribe_event(struct v4l2_fh  *fh,
...
static int armcb_v4l2_config_unsubscribe_event(struct v4l2_fh *fh,
	const struct v4l2_event_subscription *sub)
{
	struct v4l2_event ev;
	LOG(LOG_INFO, "+");
...
static void buffer_done_i7_handle(struct isp_irq_info *info)
{
	...
	if (info->status & I7_INT_3A_INT_MASK) {
		...
		if (!armcb_v4l2_find_stream_by_outport_ctx(outport, ctx_id, &stream_id)) {
			armcb_isp_put_frame(ctx_id, stream_id, ISP_OUTPUT_PORT_3A);
		}

		armcb_v4l2_config_queue_event_with_status(pvdev, info, I7_INT_3A_INT_MASK);
	...
	if (info->status & I7_INT_VIN_EXPX_INT_MASK) {
		if (info->status & I7_INT_VIN_CHNL_L_BUF_DONE) {  //Channel Long
			...
			armcb_v4l2_config_queue_event_with_status(pvdev, info, I7_INT_VIN_CHNL_L_BUF_DONE);

			info->status &= ~I7_INT_VIN_CHNL_L_BUF_DONE;
		}
		if (info->status & I7_INT_VIN_CHNL_M_BUF_DONE) {  //Channel Middle
			armcb_isp_put_frame(ctx_id, -1, ISP_OUTPUT_PORT_VIN);
			armcb_v4l2_config_queue_event_with_status(pvdev, info, I7_INT_VIN_CHNL_M_BUF_DONE);

			info->status &= ~I7_INT_VIN_CHNL_M_BUF_DONE;
		}
		if (info->status & I7_INT_VIN_CHNL_S_BUF_DONE) {  //Channel Short
			armcb_isp_put_frame(ctx_id, -1, ISP_OUTPUT_PORT_VIN);
			armcb_v4l2_config_queue_event_with_status(pvdev, info, I7_INT_VIN_CHNL_S_BUF_DONE);

			info->status &= ~I7_INT_VIN_CHNL_S_BUF_DONE;
		}
		if (info->status & I7_INT_VIN_CHNL_VS_BUF_DONE) {  //Channel v short
			armcb_isp_put_frame(ctx_id, -1, ISP_OUTPUT_PORT_VIN);
			armcb_v4l2_config_queue_event_with_status(pvdev, info, I7_INT_VIN_CHNL_VS_BUF_DONE);
...
static int armcb_v4l2_config_probe(struct platform_device *pdev)
{
...
#endif
	p_v4l_config_dev = devm_kzalloc(&pdev->dev, sizeof(armcb_v4l2_config_dev_t), GFP_KERNEL);
	...
	snprintf(p_v4l_config_dev->v4l2_dev.name, V4L2_DEVICE_NAME_SIZE*sizeof(char), "%s", ARMCB_MODULE_NAME);
	LOG(LOG_INFO, "armcb config dev name:%s", p_v4l_config_dev->v4l2_dev.name);
	ret = v4l2_device_register(&pdev->dev, &p_v4l_config_dev->v4l2_dev);
	if (ret) {
		LOG(LOG_ERR, "failed to register v4l2 device %s", p_v4l_config_dev->v4l2_dev.name);
		goto exit_ret;
	}
	...
	p_v4l_config_dev->vid_cap_caps = V4L2_CAP_VIDEO_CAPTURE_MPLANE | V4L2_CAP_STREAMING | V4L2_CAP_READWRITE;
	...
	LOG(LOG_INFO, "vfd->name:%s v4l2_dev.name:%s dev_name:%s",
			vfd->name, p_v4l_config_dev->v4l2_dev.name, video_device_node_name(vfd));

	/* register isr function*/
	if (has_acpi_companion(&pdev->dev)) {
		hw_info = (struct armcb_isp_info *)acpi_device_get_match_data(&pdev->dev);
	} else {
		hw_info = (struct armcb_isp_info *)of_device_get_match_data(&pdev->dev);
	}
...
#ifdef QEMU_ON_VEXPRESS
	interrupt_task = kthread_run(armcb_isp_interrupt_task, (void *)p_v4l_config_dev, "interrupt_thread");
	...
	if (isp_irqno <= 0) {
		ret = -ENXIO;
		LOG(LOG_ERR, "platform_get_irq failed irq(%d) ret(%d)",  isp_irqno, ret);
		goto unreg_dev;
	}

#ifdef CONFIG_ARENA_FPGA_PLATFORM
	ret = devm_request_irq(&pdev->dev, isp_irqno, hw_info->isp_isr,
			IRQF_SHARED, pdev->name, (void *)p_v4l_config_dev);
#else
	ret = devm_request_irq(&pdev->dev, isp_irqno, hw_info->isp_isr,
			IRQF_ONESHOT|IRQF_SHARED, pdev->name, (void *)p_v4l_config_dev);
#endif
	if (ret != 0) {
		ret = -ENXIO;
	...
	if (hw_info->type == ISP_TYPE_I7) {
		...
		ret = devm_request_irq(&pdev->dev, isp_irqno, hw_info->isp_err_isr,
				IRQF_SHARED, pdev->name, (void *)p_v4l_config_dev);
	...
	if (ret) {
		LOG(LOG_ERR, "cix_isp_rproc_rdr_register_core fail, ret = [%d]\n", ret);
		goto err_rproc_add;
	}
#endif
...
static const struct acpi_device_id armcb_v4l2_config_acpi_match[] = {
							{ .id = "CIXH3020",
							  .driver_data = (kernel_ulong_t)&i7_isp_hw_info },
							{ /* sentinel */ } };
